...
const Particles = (function (window, document) {
  ...
  Plugin.prototype._initializeCanvas = function () {
    const _ = this; let devicePixelRatio; let backingStoreRatio
  ...
  Plugin.prototype._checkResponsive = function () {
    const _ = this; let breakpoint; let targetBreakpoint = false; const windowWidth = window.innerWidth
  ...
  Plugin.prototype._draw = function () {
    ...
    const parentWidth = (element.offsetParent) ? element.offsetParent.clientWidth : element.clientWidth
    let parentHeight = (element.offsetParent) ? element.offsetParent.clientHeight : element.clientHeight
    const showParticles = _.options.showParticles
    const storage = _.storage
  ...
  Plugin.prototype._updateEdges = function () {
    const _ = this
    const minDistance = _.options.minDistance
    ...
    for (let i = 0; i < storageLength; i++) {
      ...
      for (let j = i + 1; j < storageLength; j++) {
        const p2 = storage[j]
        var distance; const r = p1.x - p2.x; const dy = p1.y - p2.y
  ...
  Plugin.prototype._drawEdge = function (p1, p2, opacity) {
    const _ = this
    const gradient = _.context.createLinearGradient(p1.x, p1.y, p2.x, p2.y)
  ...
  Particle = function (context, options) {
    ...
    const color = (options.color instanceof Array) ? options.color[Math.floor(Math.random() * options.color.length)] : options.color
